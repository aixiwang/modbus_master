 //--------------------------------------
// MODBUS MASTER LIB
// Copyright by Aixi Wang
//--------------------------------------
#include "modbus_lib.h"

//-------------------------------------------------------------
// func 03 steps:
//     1. create_03_pkg
//     2. send the pkg
//     3. read income package, len = read_03_resp_len()
//     4. decode_03_response
//     5. extract needed data
//
//------------------------------
// create_03_pkg
//------------------------------
int create_03_pkg(u8* buff,u8 addr,u16 start,u16 lens,u8* p_pkg_len)
{
	u16 crc;

	if ((lens * 2 +  7) > 64)
		return -1;

	buff[0] = addr;
	buff[1] = 0x03;
	buff[2] = start >> 8;
	buff[3] = start & 0xff;
	buff[4] = lens >> 8;
	buff[5] = lens & 0xff;

	crc = gen_crc_16(buff,6);
	buff[6] = crc >> 8;
	buff[7] = crc & 0xff;

	*p_pkg_len = 8;
	return 0;
}

//------------------------------
// read_03_resp_len
//------------------------------
int read_03_resp_len(int n)
{
	return 5+n*2;
}

//------------------------------
// read_03_response_pkg
//------------------------------
int decode_03_response(u8* buff,u8 addr,u8 n,u16* p_w)
{
	u16 crc;
	u8 cnt;
	int i;
	cnt = read_03_resp_len(n);

	if (buff[0] != addr)
	{
		return -2;
	}

	if (buff[1] != 0x03)
	{
		return -3;
	}

	if (buff[2] != (n*2))
	{
		return -4;
	}

	crc = gen_crc_16(buff,cnt-2);


	if (buff[cnt-2] != (crc >> 8))
	{
		return -5;
    }

	if (buff[cnt-1] != (crc & 0xff))
	{
		return -6;
    }

	 for(i=0; i<n; i++)
	 {
		*(p_w+i) = buff[3+i*2]*256 + buff[4+i*2];	
	 }


	 return 0;


}


//------------------------------
// gen_crc_16
//------------------------------
unsigned int gen_crc_16(unsigned char *ptr,  unsigned char len)
{
    unsigned int index;
    unsigned char crch = 0xFF;
    unsigned char crcl = 0xFF;
    unsigned char code TabH[] = {
          
    } ;  
    unsigned char code TabL[] = {
          
    } ;
    while (len--)
    {
        index = crch ^ *ptr++;
        crch = crcl ^ TabH[index];
        crcl = TabL[index];
    }
   
    return ((crch<<8) | crcl);  
}                           

